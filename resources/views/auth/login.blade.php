...
<div class="row justify-content-center">
      <div class="col-md-8">
        <div class="card-group mb-0">
          <div class="card p-4">
          <!-- la accion sera la sig ruta login -->
          <form class="form-horizontal was-validated" method="POST" action="{{route('login')}}">
          <!-- protege la app de solicitudes de falsificacion -->
          {{ csrf_field() }}
            <div class="card-body">
              <h1>Acceder</h1>
              <p class="text-muted">Control de acceso al sistema</p>
              <!--verifica si se lanza una exepcion en el campo usuario se agrega la clase is-invalid-->
              <!-- si el usuario no coincide -->
              <div class="form-group mb-3 {{$errors->has('usuario' ? 'is-invalid' : '')}}">
                <span class="input-group-addon"><i class="icon-user"></i></span>
                <input type="text" value="{{old('usuario')}}" name="usuario" id="usuario" class="form-control" placeholder="Usuario">
                <!-- muestra el error por ej si no se encuantra registrado -->
